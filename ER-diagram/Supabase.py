...
def create_er_diagram():
    ...
    dot.node(
        ...
        label=(
            ...
            "<TR><TD>category_id</TD><TD>UUID (FK → categories)</TD></TR>"
    ...
    dot.node(
        ...
        label=(
            ...
            "<TR><TD><I>id</I></TD><TD>UUID (PK)</TD></TR>"
            "<TR><TD>temporary_spot_id</TD><TD>UUID (FK → temporary_spots)</TD></TR>"
    ...
    dot.node(
        ...
        label=(
            ...
            "<TR><TD>temporary_spot_id</TD><TD>UUID (FK → temporary_spots)</TD></TR>"
            "<TR><TD>tag_id</TD><TD>UUID (FK → tags)</TD></TR>"
            "<TR><TD>created_at</TD><TD>TIMESTAMP WITH TIME ZONE DEFAULT now()</TD></TR>"
            "</TABLE>>"
        ),
        shape="none",
    ...
    # =============================
    # テーブル: temporary_spot_photos (photo)
    # =============================
    # SQL上では "photo" という名前ですが、用途に応じて分かりやすくノード名を変えても構いません。
    dot.node(
        "photo",
        label=(
            "<<TABLE BORDER='1' CELLBORDER='1' CELLSPACING='0' CELLPADDING='4'>"
            "<TR><TD BGCOLOR='lightblue' COLSPAN='2'><B>photo</B></TD></TR>"
            "<TR><TD><I>id</I></TD><TD>UUID (PK)</TD></TR>"
            "<TR><TD>temporary_spot_id</TD><TD>UUID (FK → temporary_spots)</TD></TR>"
            "<TR><TD>file_path</TD><TD>TEXT NOT NULL</TD></TR>"
            "<TR><TD>public_url</TD><TD>TEXT</TD></TR>"
            "<TR><TD>caption</TD><TD>TEXT</TD></TR>"
            "<TR><TD>created_at</TD><TD>TIMESTAMP WITH TIME ZONE DEFAULT now()</TD></TR>"
            "<TR><TD>file_hash</TD><TD>TEXT NOT NULL (UNIQUE)</TD></TR>"
            "</TABLE>>"
        ),
        shape="none",
    )
    ...
    dot.node(
        ...
        label=(
            ...
            "<TR><TD><I>id</I></TD><TD>UUID (PK)</TD></TR>"
            "<TR><TD>newsletter_issue_id</TD><TD>UUID (FK → newsletter_issues)</TD></TR>"
            "<TR><TD>subscriber_id</TD><TD>UUID (FK → newsletter_subscribers)</TD></TR>"
    ...
    dot.node(
        ...
        label=(
            "<<TABLE BORDER='1' CELLBORDER='1' CELLSPACING='0' CELLPADDING='4'>"
            "<TR><TD BGCOLOR='lightgreen'><B>spot_status (ENUM)</B></TD></TR>"
            "<TR><TD>'draft'</TD></TR>"
    ...
    # photo -> temporary_spots (temporary_spot_id)
    dot.edge("photo", "temporary_spots", label="temporary_spot_id → id")

    # temporary_spots.status -> spot_status
    dot.edge("temporary_spots", "spot_status", label="status → (ENUM)")

    # ER 図の描画
    dot.render("Supabase", cleanup=True)
